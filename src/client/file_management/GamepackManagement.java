...
import client.file_management.PropertyManagement;
 ...
 */
public class GamepackManagement {
    ...
    public static int getLocalRevision() {
        if (!PropertyManagement.loadProperties(DirectoryFolder.DATA.getDirectoryPath() + File.separator + DirectoryFolder.DATA.getFolderName(), File.separator + DirectoryFile.CLIENT_BUILD.getFileName(), DirectoryFile.CLIENT_BUILD.getFileExtension()))
            return -1;

        if (!PropertyManagement.getFileProperties().getProperty("local_revision").isEmpty()) {
            try {
                return Integer.parseInt(PropertyManagement.getFileProperties().getProperty("local_revision"));
    ...
    public static void requestGamepack() {
        ...
        if (Request.getFile(getConfigParameters().get("codebase") + getConfigParameters().get("initial_jar"), GAMEPACK_PATH, DirectoryFile.GAMEPACK.getFileName(), DirectoryFile.GAMEPACK.getFileExtension(), true))
            putRevision();
    }
    ...
    public static boolean needsGamepack() {
        ...
        final int GAMEPACK_SIZE = Request.getFileSize(getConfigParameters().get("codebase") + getConfigParameters().get("initial_jar"));

        final String GAMEPACK_PATH = DirectoryFolder.DATA.getDirectoryPath() + File.separator + DirectoryFolder.DATA.getFolderName();
        final File CURRENT_GAMEPACK = FileManagement.getFileInDirectory(GAMEPACK_PATH, DirectoryFile.GAMEPACK.getFileName(), DirectoryFile.GAMEPACK.getFileExtension());
    ...
    public static void putRevision() {
        ...
        PropertyManagement.putProperty(PROPERTY_NAME, REVISION, FILE_PATH, FILE_NAME, FILE_EXTENSION);
    }
